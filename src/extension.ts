...
function createLanguageClient(includeArgs: string[]) {
    ...
    const fstarClient = new LanguageClient('vsfstar', 'F* Language Server', serverOptions, clientOptions, true);
    return fstarClient;
}

async function findFStarIncludeDirectories(rootPath: string): Promise<string[]> {
    let includeArgs: string[] = [];
    try {
        const output = execSync('git rev-list --max-parents=0 @', { cwd: rootPath }).toString();
        // Identify FStar.git using the SHA1 of the very first commit
        if (output.startsWith('05758a0e58a1e')) {
            const argsForHacking = includeArgsForCompilerHacking(rootPath);
            includeArgs = includeArgs.concat(argsForHacking);
            docSel.push({ scheme: 'file', language: 'fsharp' });
        }
    } catch(_) {
        // Ignore all errors during attempt to check for FStar repository.
    }
    ...
    if (includeDirGlobPattern.length >= 1 && includeDirGlobPattern[includeDirGlobPattern.length - 1] !== "/") {
        ...
        const globSearch = promisify(glob);
        const dirList = await globSearch(includeDirGlobPattern, { cwd: rootPath }).catch(err => {
            console.error(err);
...
let languageServerSubscription: vscode.Disposable | null = null;
let docSel: { scheme: string; language: string; }[] = [];
    
export async function activate(context: vscode.ExtensionContext) {
    const rootPath = vscode.workspace.workspaceFolders[0].uri.fsPath;
    const includeArgs = await findFStarIncludeDirectories(rootPath);
    let fstarClient = createLanguageClient(includeArgs);

    console.log('F* Language Server is now active!');
    languageServerSubscription = fstarClient.start();
    context.subscriptions.push(languageServerSubscription);
